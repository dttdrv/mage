...
Root cause — CONFIRMED 2026-08-02: the fast-path build was made from the
main-project worktree `upstream/MoltenVK-ray-as-minimal`, which contains
NONE of the magevk Mage hooks: no MAGE_VK_* property spoof, no
MAGE_MVK_ENABLE_PRIVATE_SPARSE_BUFFERS private sparse buffers, no fake
feature flags (geometryShader/shaderCullDistance). So the launcher saw an
unspoofed Apple integrated GPU — vendorID 0x106B, non-discrete device
type, no sparse features — and the §6 gate chain rejected it exactly as
it rejects any unsupported GPU. It was never a feature-bit delta.
Evidence: Appendix E3 (strings counts: 0 Mage-hook symbols in the
fast-path dylib vs 2 in the magevk build; 0 grep matches for the hook
env vars in the upstream/MoltenVK-ray-as-minimal source tree).

Fix in progress: the 4 RT commits from the fast-path line
(d5f35ba5..25440e4e) are being merged into mage/magevk separately, so the
fast path lands on top of the Mage hooks instead of replacing them.
...
4. TDA perf: "super slow" + flicker (user report from a brief session;
   nothing more has been user-confirmed). Fast-path build was the hope —
   root cause confirmed (Part 2 §10), RT commits being merged into magevk.
