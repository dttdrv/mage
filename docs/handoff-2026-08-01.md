...
ray_tracing_pipeline enabled, submits work, and presents frames. One
...
Later same day the game became genuinely playable (user played the
campaign); current perf complaint is "super slow" + FPS dips, and the
in-scene corruption (blocky garbage over 3D, UI clean) still stands with
the same two suspects (private sparse buffers, BLAS nativeSize growth /
retainCurrentGeneration).

## 10. Fast-path MoltenVK (25440e4e) FAILS the launcher GPU gate (2026-08-01)
...
- So the delta is in feature/property bits or device identity
  (deviceUUID/LUID vs the D3DKMT EnumAdapters2 match — the launcher's
  check chain in §6). A probe to diff feature bits was attempted but the
  quick C probe kept crashing in vkGetPhysicalDeviceFeatures2 (probe bug,
  not the dylib) — needs a proper capture (gfxrecon or GPT's own tooling)
  against both builds.
...
3. TDA second-alt-tab black screen: present/drawable reacquisition,
   untested.
4. TDA perf: "super slow", FPS dips in congested areas. Fast-path build
   was the hope — blocked by Part 2 §10.
